...
- Keep responses concise but informative`;
    ...
    try {
      ...
      return buildAiLimitMessage(error);
    }
  }
}

function buildAiLimitMessage(error: any): string {
  const msg = error?.message || '';
  const status = error?.status;
  const isRateLimit = status === 429 || msg.includes('429') || msg.includes('Too Many Requests') || msg.includes('quota');
  const isOverloaded = status === 503 || msg.includes('503') || msg.includes('Service Unavailable') || msg.includes('high demand');
